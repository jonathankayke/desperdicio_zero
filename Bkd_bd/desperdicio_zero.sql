-- Ti 19 
-- Backup Geral do banco de dados desperdicio_zero 
-- Excluir o usuário desperdicio_zero caso ele exista 
DROP USER IF EXISTS 'iwanez83_desperdicio_zero'@'localhost';

-- Criar o usuário desperdicio_zero se ele não existir 
CREATE USER IF NOT EXISTS 'iwanez83_desperdicio_zero'@'localhost' 
    IDENTIFIED BY 'senacti19';

GRANT ALL PRIVILEGES 
ON iwanez83_desperdicio_zero.* 
TO 'iwanez83_desperdicio_zero'@'localhost';
    

-- Excluir o banco de dados desperdicio_zero caso ele exista 
DROP DATABASE IF EXISTS iwanez83_desperdicio_zero; 

-- Criar o banco de dados desperdicio_zero se ele não existir 
CREATE DATABASE IF NOT EXISTS iwanez83_desperdicio_zero     
    DEFAULT CHARACTER SET utf8 
    COLLATE utf8_general_ci;

-- Usamos o banco de dados desperdicio_zero 
USE iwanez83_desperdicio_zero;

-- Tabela de usuários 
CREATE TABLE tbusuarios (
    id_usuario INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    nome_usuario VARCHAR(150) NOT NULL,
    email_usuario VARCHAR(120) NOT NULL UNIQUE,
    login_usuario VARCHAR(100) NOT NULL UNIQUE,
    senha_usuario VARCHAR(255) NOT NULL,
    tipo_usuario ENUM('Admin', 'User') DEFAULT 'User',
    foto_usuario VARCHAR(255) NULL
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Estrutura da tabela tbtipos    
CREATE TABLE tbtipos (
    id_tipo INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
    sigla_tipo VARCHAR(3) NOT NULL,
    rotulo_tipo VARCHAR(15) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO tbtipos (sigla_tipo, rotulo_tipo)
VALUES
('F','Fruta'),
('V','Verdura'),
('G','Grão'),
('P','Proteína'),
('L','Laticínio'),
('B','Bebida'),
('C','Conserva'),
('M','Massa');

-- Tabela de tbdoações 
CREATE TABLE tbdoacoes (
    id_doacao INT(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
    id_doacao_tipo INT(11) NOT NULL,
    nome_empresa VARCHAR(150) NOT NULL,
    contato_doacao VARCHAR(150) NOT NULL,
    tipo_alimento VARCHAR(150) NOT NULL,
    nome_alimento VARCHAR(150) NOT NULL,
    quantidade_doacao VARCHAR(50) NOT NULL,
    validade_doacao DATE,
    endereco_retirada VARCHAR(150) NOT NULL,
    imagem_doacao VARCHAR(150) NOT NULL,
    INDEX (id_doacao_tipo),
    CONSTRAINT id_doacao_tipo_fk
        FOREIGN KEY (id_doacao_tipo)
        REFERENCES tbtipos(id_tipo)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO tbdoacoes (id_doacao_tipo,nome_empresa,contato_doacao,tipo_alimento,nome_alimento,quantidade_doacao,validade_doacao,endereco_retirada,imagem_doacao)
 VALUES (1,'Boa Vista Frutas','1111-1111','F','Maçã','10 kg','2026-03-15','Rua A, 123','macas.webp'),
  (2,'Venduras Do Jaime','2222-2222','V','Alface','5 kg','2026-03-01','Rua B, 456','alface.webp'),
  (3,'Cirilo Atacadista','3333-3333','G', 'Feijão','20 kg','2026-05-05','Rua C, 789','feijao.webp'),
  (4,'Açougue da Familia','4444-4444','P','Frango','15 kg','2026-03-05','Rua D, 101','frango.webp'),
  (5,'Mercado Lobinho','5555-5555','L','Leite','30 litros','2026-04-18','Rua E, 202','leite.webp'),
  (6,'Atacado das Bebidas','6666-6666','B','Suco de Laranja','25 litros','2026-03-22','Rua F, 303','suco_de_laranja.webp'),
  (7,'Paulo Pizzaria','3333-3333','C','Milho Enlatado','50 unidades','2026-06-10','Rua C, 789','milho.webp'),
  (1,'Boa Vista Frutas','7777-7777','F','Tomate','10 kg','2026-02-28','Rua B, 189','tomate.webp'),
  (3,'Cirilo Atacadista','8888-8888','G','Arroz','15 kg','2026-05-12','Rua O, 1449','arroz.webp'),
  (8,'Italian Restaurant','9999-9999','M','Macarrão','12 kg','2026-04-23','Rua F, 144','macarrao.webp');

-- Inserindo usuário padrão para teste
INSERT INTO tbusuarios (nome_usuario, email_usuario, login_usuario, senha_usuario, tipo_usuario, foto_usuario)
VALUES ('Administrador', 'admin@desperdiciozero.com', 'admin', '123456', 'Admin', 'Chefe.webp'),
       ('Usuário', 'usuario@desperdiciozero.com', 'user', '123', 'User', 'Secretaria.webp');

-- ----- AUTO INCREMENTS -----
ALTER TABLE tbusuarios AUTO_INCREMENT=3;
ALTER TABLE tbdoacoes MODIFY id_doacao INT(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;
ALTER TABLE tbtipos AUTO_INCREMENT=10;


-- -------- VIEW -------- -- Criando a view vw_tbdoacoes 
CREATE VIEW vw_doacoes AS
SELECT  d.id_doacao,
        d.nome_empresa,
        d.id_doacao_tipo,
        d.nome_alimento,
        d.quantidade_doacao,
        d.validade_doacao,
        d.endereco_retirada,
        d.imagem_doacao,
        d.contato_doacao,
        t.sigla_tipo,
        t.rotulo_tipo
FROM    tbdoacoes d LEFT JOIN tbtipos t ON d.id_doacao_tipo = t.id_tipo;